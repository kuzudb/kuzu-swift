    ...
    throws -> UnsafeMutablePointer<kuzu_value>
{
    ...
    var cElementArray: UnsafeMutablePointer<UnsafeMutablePointer<kuzu_value>?> =
        .allocate(capacity: numberOfElements)
    for (idx, element) in array.enumerated() {
        let cElement = try swiftValueToKuzuValue(element)
        let cString: UnsafeMutablePointer<CChar> = kuzu_value_to_string(
            cElement
        )
    ...
    var cKuzuListValue: UnsafeMutablePointer<kuzu_value>? = .allocate(
        capacity: 1
    )
